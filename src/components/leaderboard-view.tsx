  ...
  return (
    <div>
      ...
      <div className="overflow-hidden rounded-2xl border border-line bg-pitch-900/40">
        {items.map(({ p, pts, rank }) => {
          const isMe = p.user_id === myId;
          const rankColor =
            rank === 1 ? "text-gold" : rank === 2 ? "text-ink" : rank === 3 ? "text-coral" : "text-muted";
          return (
            <div
              key={p.user_id}
            ...
            >
              <span className={`w-6 text-center font-display text-lg ${rankColor}`}>{rank}</span>
